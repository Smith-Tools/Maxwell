...
test:
	...
	@[ -f "$(LOCAL_AGENT_DIR)/maxwell-tca.md" ] && echo "   ✅ maxwell-tca agent valid"
	@[ -f "$(LOCAL_AGENT_DIR)/maxwell-shareplay.md" ] && echo "   ✅ maxwell-shareplay agent valid"
	@echo "   ✅ All agent structures valid"
	@echo ""
	@echo "   🧪 Testing database connectivity..."
	@cd "$(LOCAL_DB_DIR)"
	@if [ -f "maxwell.db" ]; then
		sqlite3 maxwell.db "SELECT COUNT(*) as total_patterns FROM patterns;" >/dev/null && echo "   ✅ Database connectivity working" || echo "   ⚠️ Database connectivity issue"
	else
