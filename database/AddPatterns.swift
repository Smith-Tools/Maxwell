...
// Import SimpleDatabase by including the source
// For now, we'll create patterns that can be manually added later
...
print("🔍 TCA Patterns Extracted from Documentation")
...
for (_, pattern) in patterns.enumerated() {
    ...
    print("   Solution: \\(pattern.solution)")
    if !pattern.codeExample.isEmpty {
        print("   Code Example:")
        print("   ```swift\\(pattern.codeExample)\\n   ```")
    }
    print("   Notes: \\(pattern.notes)")
    print(String(repeating: "-", count: 30))
}

print("\\n📊 Summary: \\(patterns.count) TCA patterns extracted")
print("\\n📝 Next Steps:")
print("   1. Add these patterns to Maxwell database using the pattern insertion methods")
print("   2. Test pattern search functionality")
print("   3. Validate canonical source references")