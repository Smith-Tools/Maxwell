...
## 🚨 CRITICAL: TYPE INFERENCE ANTI-PATTERNS

**[CRITICAL]** Before implementing any TCA patterns, understand these compilation-destroying anti-patterns:

### TCA Type Inference Explosion

**❌ ANTI-PATTERN**: Nested `CombineReducers` - causes infinite compilation hangs
```swift
// NEVER DO THIS - causes type inference explosion
var body: some Reducer<State, Action> {
  CombineReducers {
    BindingReducer()
    CombineReducers {          // NESTED = COMPILATION HANG
      Scope(state: \.child, action: \.child) { ChildFeature() }
      mainReducer()           // Function returning more CombineReducers
    }
  }
}
```

**✅ OPTIMAL PATTERN**: Flat `@ReducerBuilder` composition
```swift
// ALWAYS DO THIS - compiles in seconds
var body: some Reducer<State, Action> {
  BindingReducer()
  ...
  // Flat logical grouping - NO nested CombineReducers
  dataFlowReducers()
  uiBindingReducers()
  contentManagementReducers()
}

// Helper functions for organization (never nested CombineReducers)
@ReducerBuilder<State, Action>
private func dataFlowReducers() -> some Reducer<State, Action> {
  Reduce { state, action in
    // Simple logic - no complex nesting
    return .none
  }
}
```

**KEY INSIGHT**: The problem isn't the NUMBER of reducers, but **implicit composition structure**. Flat composition eliminates type inference chains.
